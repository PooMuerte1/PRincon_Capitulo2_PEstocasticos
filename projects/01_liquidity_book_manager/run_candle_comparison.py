...
def downsample_dataframe(df: pd.DataFrame, k: int) -> pd.DataFrame:
    """
    Agrupa cada k registros de alta frecuencia (1 minuto) en una sola vela de resolución más baja (k minutos).
    """
    records = []
    n = len(df)
    for i in range(0, n, k):
        ...
        # El precio de la vela es el último precio de cierre del bloque
        price_close = chunk["price"].values[-1]
        
        # La volatilidad promedio del bloque
        vol_mean = chunk["volatility_true"].mean()
        
        # El volumen es la suma total acumulada de swaps en el bloque
...
def run_candle_comparison_campaign():
    ...
    print(f"Cargando dataset histórico empírico base: {csv_path}")
    print(f"  - Registros base: {len(df_hist):,} velas de 1 minuto.\n")

    backtester = BinLiquidityBacktester()
    
    # Calcular valor del HODL Puro 50/50
    p_initial = df_hist["price"].values[0]
    p_final = df_hist["price"].values[-1]
    hodl_value = capital * 0.5 + capital * 0.5 * (p_final / p_initial)
    
    # Definir resoluciones a evaluar (k minutos por vela)
    resolutions = [
        ("Velas de 1 Minuto (Frecuencia Máxima)", 1, 1.0 / 525600.0),
        ("Velas de 5 Minutos (Frecuencia Media)", 5, 5.0 / 525600.0),
        ("Velas de 15 Minutos (Frecuencia Baja)", 15, 15.0 / 525600.0)
    ]

    for title, k_factor, dt_step in resolutions:
        # Downsampling del DataFrame
        if k_factor == 1:
            df_res = df_hist.copy()
        else:
            df_res = downsample_dataframe(df_hist, k_factor)
        ...
        # El volumen real en el downsampled es la suma por bloque,
        # así que para expected_volume_per_minute dividimos el volumen promedio entre k_factor.
        expected_vol_min = volume.mean() / k_factor
        
        price_change = ((prices[-1] / prices[0]) - 1.0) * 100
        total_vol = volume.sum()
        
        stats_str = (
            f"Resolución: Velas de {k_factor}m | Duración: {n_steps * k_factor / 1440:.2f} días | "
        ...
        # Imprimir la tabla para esta resolución
        print_multi_period_table(
            f"COMPARACIÓN DE RESOLUCIÓN: {title.upper()}",
            stats_str,
            results,
            capital,
            hodl_value
        ...
        # Guardar gráfico de rendimiento
        output_filename = f"visual_candle_comparison_{k_factor}m.png"
        output_path = os.path.join(os.path.dirname(__file__), output_filename)
        save_performance_plot(
            times=times,
            results=results,
            output_path=output_path,
            title=f"Evolución de Rentabilidad sobre HODL - Velas de {k_factor} Minutos"
        )
        print(f"¡Gráfico de rendimiento guardado en: {output_path}!\n")

if __name__ == "__main__":
    run_candle_comparison_campaign()
